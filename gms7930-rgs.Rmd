...
Thus, R packages chosen here are for demostration purpose, and might not be well adopted by community users.
...
is to demostrate ChIP-seq peak detection and annotation. (2) tools for raw data processing, *e.g.* reads mapping,
...
sequencing data can be downloaded from here: <https://hgdownload.soe.ucsc.edu/goldenPath/hg19/encodeDCC/wgEncodeBroadHistone/>.
As we discussed before, we also need INPUT sample to control experimental noises. We can download both files
with the following code and put all files into a folder for downstream analysis. Note, we also download the 
...
workdir = '~/' ## note: For Windows 10/8/7, please use "C:\\" instead of "~/"
...
With the downloaded dataset, we can compare ChIP and INPUT to see if ChIP actually contains 
...
purpose, we use large window size to mimic peak regions. Then, we can apply simple normalizatin 
...
```{r bam, echo=TRUE}
## sequencing files that only contrain reads from chromosome 21
chip = file.path(workdir,'Gm12878Ctcf.bam')
input = file.path(workdir,'Gm12878Control.bam')
## chromosome length info from humange genome build hg19
hg_chrs = fetchExtendedChromInfoFromUCSC('hg19') 
hg_chrs_chr21 = subset(hg_chrs, grepl('chr21$',UCSC_seqlevel))
seqlen_chr21 = with(hg_chrs_chr21, setNames(UCSC_seqlength, UCSC_seqlevel))
## create genomic 1000bp windows and store them using GenomeRanges
...
plot(log2(cpm_chr21[,2]+1),log2(cpm_chr21[,1]+1),pch=16,cex=0.25,xlab='log2(INPUT)',ylab='log2(ChIP)')
abline(a=0,b=1)
## read counts along the chromosome
plot(log2(cpm_chr21[,2]+1),col='blue',pch=16,cex=0.25,xlab='window index',ylab='log2 signal')
points(log2(cpm_chr21[,1]+1),col='red',pch=16,cex=0.25)
...
general model fitting results, inlcuding the percentage of enriched windows and the q-values.

```{r normr,echo=TRUE,message=FALSE}
## peak deteaction based on 250bp windows
peakfit = enrichR(treatment = chip,control=input,genome='hg19',verbose=FALSE)
summary(peakfit)
```

We can see that the total number of significant enriched peaks/windows is 1154, spreading across
...
peaks$qvalue = getQvalues(peakfit) ## add enrichment significance
peaks
```

We can see most of the peaks/windows hold q-values as NA. These regions are actually not significantly 
enriched based on function **enrichR**. However, all regions are reported no matter the statuses of their 
significance. Thus, we should further filter these peaks using q-values.

```{r peaksig, echo=TRUE, message=TRUE}
## only consider a peak as significantly enriched if its q-value is less than 0.01
peakssig = peaks[which(peaks$qvalue<0.01)]
peakssig = peakssig[order(peakssig$qvalue)]
peakssig
```

After filtering based on q-values, we have resulted 940 enriched peaks/windows. Now we can go back 
...
plot(log2(cpm_chr21[-sigidx,2]+1),log2(cpm_chr21[-sigidx,1]+1),pch=16,cex=0.25,xlab='log2(INPUT)',ylab='log2(ChIP)')
points(log2(cpm_chr21[sigidx,2]+1),log2(cpm_chr21[sigidx,1]+1),pch=16,cex=0.25,col='red')
abline(a=0,b=1)
```
...
across integenic regions, promoters, introns and exons etc.
